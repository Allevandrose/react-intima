...
const ProductDetail = () => {
  ...
  if (!product) {
    return (
      <>
        <SEO
          title="Product Not Found | Intimacare"
          description="The product you're looking for could not be found. Browse our collection of premium adult toys in Kenya."
          keywords="product not found, adult toys Kenya"
  ...
  // Prepare SEO data
  const seoTitle = `${product.name} | Intimacare Kenya`;
  const seoDescription =
    product.description?.substring(0, 160) ||
    `Buy ${product.name} at Intimacare Kenya. Premium adult toys with discreet shipping across Kenya.`;
  const seoKeywords = `${product.name}, ${product.category || "adult toys"}, buy ${product.name} Kenya, adult toys Kenya, sex toys Nairobi, Intimacare`;
  const seoImage = product.images?.[0] || "/favicon.svg";
  const seoUrl = `/product/${product.slug || product._id}`;
